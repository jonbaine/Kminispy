...
--*/
{
	...
	if (RecordData->RecordType == 0)
	{
		...
		//Only log the IRP_CREATE (open), IRP_MJ_SET_INFORMATION (delete, rename...), IRP_MJ_WRITE, IRP_MJ_READ and IRP_MJ_CLOSE (close the file.)
		if (!
		...
		{
			if (GetProcessImageFileNameA(procHandle, (LPSTR)pName, 512) > 0)
				printf(" \"%s\"\t", pName);
			else
				printf(" [NO_EXE]\t");
	...
	else if (RecordData->RecordType == 1)
	{
		...
		//PARENT!
		if (procHandle = OpenProcess(PROCESS_QUERY_LIMITED_INFORMATION, FALSE, (DWORD)RecordData->ThreadId))
		{
			if (GetProcessImageFileNameA(procHandle, (LPSTR)pName, 512) > 0)
				printf("PARENT %d \"%s\"\t", RecordData->ThreadId, pName);
			else
